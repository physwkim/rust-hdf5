...
// write_raw a whole many-chunk deflate-compressed fixed-array dataset (bounded
// shape, no unlimited dimension). This drives the fixed-array batch path, where
// the filter pipeline compresses chunks in parallel under `--features parallel`.
fn bench_compressed_write_raw_fixed(c: &mut Criterion) {
    let mut group = c.benchmark_group("compressed_write_raw_fixed");
    for &nframes in &[100usize, 1000] {
        let ncols = 1000usize;
        let total_bytes = nframes * ncols * 8;
        let data: Vec<f64> = (0..nframes * ncols).map(|i| i as f64).collect();
        ...
        group.bench_with_input(
            ...
            |b, &nframes| {
                b.iter(|| {
                    let path = std::env::temp_dir().join("bench_compressed_write_raw_fixed.h5");
                    let file = H5File::create(&path).unwrap();
                    // No max_shape -> no unlimited dim -> fixed-array index.
                    let ds = file
                        .new_dataset::<f64>()
                        .shape([nframes, ncols])
                        .chunk(&[1, ncols])
...
criterion_group!(
    ...
    bench_compressed_write_raw_fixed,
);
