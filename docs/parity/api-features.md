...
| Alignment (`H5Pset_alignment`: threshold + alignment) | H5Ppublic.h:4205-4228 | Missing (configurable); Partial (alignment fixed by the file's strategy) | src/io/allocator.rs:91 (`with_policy`), :117 (`draw_alignment`) — eight bytes unpaged, and one page for a paged file's large manager as `H5MF__open_fstype` gives it (H5MF.c:325-330); no threshold and no public setter either way | M |
...
| File space strategy (`H5Pset_file_space_strategy`, `H5F_FSPACE_STRATEGY_*`) | H5Fpublic.h:191-204; H5Ppublic.h:3107-3126 | Implemented (all four strategies) | src/file.rs:1315 (`H5FileOptions::file_space(strategy, persist, threshold)`), :1352 (`file_space_page_size` = `H5Pset_file_space_page_size`), :644 (`tracked_free_space`); src/format/messages/superblock_ext.rs:133 (`FileSpaceStrategy`), :206 (`FileSpaceInfoMessage`, v0 and v1); `persist` writes the managers on close (src/io/writer.rs:9012 `write_free_space_managers`, settled as `H5MF_settle_meta_data_fsm` settles them at :9130) and reuses them on reopen (:5051 `reopen_free_space`); `PAGE` pages allocation through src/io/allocator.rs:190 (`allocate_paged`). What is still missing under `FSM_AGGR`/`AGGR` is the aggregator layer itself, not the managers | M |
