...
| Fill time (`H5Pset_fill_time`) | H5Ppublic.h:6677 | Missing | No `fill_time` setter on `DatasetBuilder`; `fill_write_time` is `FILL_TIME_IFSET` everywhere, the `H5D_CRT_FILL_TIME_DEF` default: src/io/writer.rs:14314,14321; src/format/messages/fill_value.rs:46 | M |
