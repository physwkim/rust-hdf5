...
| FS.8 | Free-space section classes (`H5MF_FSPACE_SECT_CLS_*` / `H5HF_FSPACE_SECT_CLS_*`) | `H5MFsection.c:81,107,133`; `H5HFsection.c:154,183,209,238` | Partial — the two file-level classes only | Partial — the two file-level classes only | `src/format/free_space.rs:102` (`FreeSpaceClass::{Metadata,RawData}` = `H5FD_MEM_SUPER`/`H5FD_MEM_DRAW`), `:122` (`message_slot`: slots 0 and 2). The fractal-heap section classes (`H5HFsection.c`) have no counterpart — heap free space is not tracked | M |
| FS.9 | Strategy `FSM_AGGR` (upstream default: FSM + small aggregators) | `H5Fpublic.h:196` | Implemented | Partial — the FSM half is real and persists; there are no small/large aggregators, so allocation sizes never route through an aggregator first | `src/file.rs:1313`; `src/io/writer.rs:8984`, `:5037` | M |
...
| FS.11 | Strategy `PAGE` (paged aggregation) | `H5Fpublic.h:198`; `H5MFaggr.c` page-alignment logic | Partial — a paged file's managers are not reopened (`reopen_free_space` returns `None` for `Page`), so its recorded space is not reused | Refused — `H5FileOptions::file_space(Page, ..)` errors rather than writing a file it cannot honor | `src/io/writer.rs:3915` (refusal), `:5037` (reopen bail); allocator has one fixed 8-byte alignment, no page-size concept | M |
| FS.12 | Strategy `NONE` (no cross-close reuse) | `H5Fpublic.h:201` | Implemented | Implemented as recorded — still **stronger than real NONE within one open session**, since freed blocks are reused in-process | `src/io/allocator.rs` (`take_free` best-fit) | M |
| FS.13 | Persisting free space across close/reopen (fsinfo `persist` flag) | `H5Ofsinfo.c:121,178,245`; default `H5F_FREE_SPACE_PERSIST_DEF = false` | Implemented — a reopen reads both managers' sections back into the allocator's free list | Implemented — each manager's header and sections are written on close and its address recorded in the fsinfo message; `persist` defaults false, as upstream | `src/io/writer.rs:8984` (`write_free_space_managers`), `:5037` (`reopen_free_space`); `src/file.rs:644` (`tracked_free_space`) | M |

**Notes.** What remains is aggregation, not persistence. Paged aggregation is the one strategy this port
refuses outright rather than approximating, so no file claims it. Reuse is also unpartitioned: `take_free`
best-fits across the whole list, where `H5MF_alloc` searches only the manager for the requested
`H5FD_mem_t`, so a metadata request can land in a region libhdf5 would have kept for raw data. That
changes which byte a block comes from, not whether the file accounts for it — `h5stat -S` reports zero
unaccounted space on files this writer produces.
...
silently dropped (§7 GRP.1); fletcher32 filter checksum stripped without verification (§3a MSG.7a); paged
aggregation refused rather than implemented, and free-list reuse unpartitioned by memory type (§6 FS.8,
FS.11 — a metadata request can take a region libhdf5 would have reserved for raw data).
